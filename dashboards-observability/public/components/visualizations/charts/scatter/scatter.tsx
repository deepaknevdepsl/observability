...
export const Scatter = ({ visualizations, layout, config }: any) => {
  ...
  const dataConfigTab = visualizations.data?.rawVizData?.scatter?.dataConfig && visualizations.data.rawVizData.scatter.dataConfig;
  ...
  const mode = dataConfig?.chartStyles?.style || 'markers';
  ...
  const [calculatedLayout, lineValues] = useMemo(() => {
    ...
    let calculatedScatterValues = valueSeries.map((field: any, index: number) => {
    ...
    if (dataConfig.thresholds || availabilityConfig.level) {
      ...
      calculatedScatterValues = [...calculatedScatterValues, thresholdTraces];
    }
    return [mergedLayout, calculatedScatterValues];
